...
OP_CONSTANT_ICONV, \
OP_CONSTANT_ONE, \
OP_CONSTANT_POOL = list(range(26))
...
PM_PERM, \
PM_OUTSHUFFLE, \
PM_MERGE_GCONV_COUNT = list(range(15))

AC_MODE_NONE, \
AC_MODE_SIGMOID, \
AC_MODE_RELU, \
AC_MODE_TANH = list(range(4))

PD_MODE_SAME, \
PD_MODE_VALID = list(range(2))
...
for d in list(operator_data.values()):
    for i in range(d[3]):
        name = '{}_{}'.format(d[0], i)
...
N = [3,4] # change this to control number of combinations for symbolic validation, e.g., [1,2], [1,3] or [3,4] each provide a reasonable experiment to run and go for coffee (assuming 8 cores)
D = [1,3]
